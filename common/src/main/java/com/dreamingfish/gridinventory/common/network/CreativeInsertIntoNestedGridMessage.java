...
public record CreativeInsertIntoNestedGridMessage(ResourceLocation itemId, int count, NestedContainerPath targetOwnerPath,
    ...
    public static void handle(CreativeInsertIntoNestedGridMessage message, GridMessageContext context) {
        if (context.player().containerMenu instanceof GridInventoryMenu menu
                && menu.creativeInsertIntoNestedGrid(message.itemId(), message.count(), message.targetOwnerPath(),
