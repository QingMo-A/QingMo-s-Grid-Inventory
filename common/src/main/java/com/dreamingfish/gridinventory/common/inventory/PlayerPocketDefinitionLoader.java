...
public class PlayerPocketDefinitionLoader extends SimpleJsonResourceReloadListener {
    ...
    protected void apply(Map<ResourceLocation, JsonElement> objects, ResourceManager resourceManager, ProfilerFiller profiler) {
        ...
        if (definition == null) {
            DFGridInventory.LOGGER.info("Loaded default player pocket definition from config");
        } else {
            DFGridInventory.LOGGER.info("Loaded player pocket definition {}", definition.id());
        }
    }
    ...
    private static Optional<EquipmentStorageContainerDefinition> parse(ResourceLocation id, JsonElement json) {
        return EquipmentStorageContainerDefinition.CODEC.parse(JsonOps.INSTANCE, json)
                .resultOrPartial(message -> DFGridInventory.LOGGER.warn("Invalid player pocket definition in {}: {}", id, message));
    }
}
