...
public final class BackpackFoldingManager {
    ...
    public static int foldedWidth(ItemStack stack) {
        return getDefinition(stack).map(BackpackFoldingDefinition::foldedWidth).orElse(GridInventoryConfig.FOLDED_BACKPACK_WIDTH.get());
    }

    public static int foldedHeight(ItemStack stack) {
        return getDefinition(stack).map(BackpackFoldingDefinition::foldedHeight).orElse(GridInventoryConfig.FOLDED_BACKPACK_HEIGHT.get());
    }
